...
| 📥 导入 | 两种入口：拍照（Qwen-VL OCR → 结构化 Markdown + 插图边界框）或直接从 BGG 拉描述 |
| 📖 规则书 | 分页阅读，TOC 默认收起，关键字高亮，原图缩放，OCR 检测到的插图随段落原位渲染 |
| 🎤 问规则 | 语音/文字提问，混合检索（向量 + FTS5+jieba），MiniMax 流式回答 + 页码引用，可选朗读 |
| 🪄 新手向导 | 对话式教练：一次发一步指令，等玩家「好了」再继续，全程支持「我有问题」打断 |
| ⚙️ 设置 | API Key（含 ElevenLabs）、TTS 语言/服务商、检索条数 K、深色模式 |
